...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict) -> None:
    ...
    for idx in bestmatch_results['indices']:
        fig.add_trace(go.Scatter(x=np.arange(idx, idx + query_len), y=ts[idx:idx + query_len], line=dict(color=px.colors.qualitative.Plotly[1], width=4)), row=1, col=2)
    ...
    fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor='rgba(0,0,0,0)', showlegend=False, title_x=0.5)
